...
FOR /F "tokens=2 delims=:{}" %%i in ('type %~dp0login.json') do wget.exe -q "http://%webhost%/xes-manager/Command/export?download=xesmanager_configuration.zip&configModules=Agents+Configuration,Alert rules&SessionId=%%i" -O %filename%
